...
7. **Brand fidelity is mandatory**: Match output style to the selected brand guideline before finalizing.

## Brand Resolution and Compliance

This plugin must support multiple brands over time.

### Brand Resolution Protocol (Scalable)

Resolve brand in this order:

1. Explicit user input (`brand`, team name, or instruction)
2. Strong context signals (client/team context, project naming, prior confirmed brand)
3. Available guideline files (for example `*-guidelines.md`) when accessible
...
### Brand Guideline Loading

After selecting a brand:
- Load the corresponding brand guideline document if available.
- Follow logo, color, typography, tone, and layout rules from that guideline.
- If required brand specs are missing, ask the user for those specifics before final delivery.
...
- selected brand id/name
- guideline source used
- required style rules applied
- unresolved brand assumptions disclosed

If any of the above is uncertain, ask a clarifying question and stop short of final output.
...
### Step 1A: Confirm Brand

Before continuing analysis, confirm the brand using the Brand Resolution Protocol.
If unclear, ask the user and pause.
...
### Brand Applied
- Brand: [brand id/name]
- Guideline source: [guideline file/path or user-provided rules]
- Brand caveats: [any missing specs, assumptions, or "none"]
...
- [ ] Brand is explicitly confirmed
- [ ] Brand guideline is loaded and applied
- [ ] Any unresolved brand assumptions are called out
